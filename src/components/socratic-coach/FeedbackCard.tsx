...
const markdownComponents = {
    p: ({ children }: { children?: React.ReactNode }) => (
        <p className="premium-text text-sm leading-relaxed text-neutral-300 font-light">{children}</p>
    ),
...
export function FeedbackCard({ feedback }: { feedback: FeedbackMessage }) {
    ...
    return (
        <motion.div
            ...
            className={`relative p-6 rounded-3xl border ${isCorrect
                ? 'bg-emerald-500/5 border-emerald-500/20 shadow-[0_0_50px_rgba(52,211,153,0.05)]'
                : 'bg-neutral-900 border-neutral-800'
                }`}
        >
            <div className="flex gap-4">
                <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 border ${isCorrect
